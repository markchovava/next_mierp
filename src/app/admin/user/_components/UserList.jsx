...
import { _userDeleteApiAction, _userListApiAction, _userPaginationApiAction, _userSearchApiAction } from 
'@/actions/UserActions';
...
export default function UserList({ dbData, subsidiaryData, roleData }) {
  ...
  async function getSearchData() {
      if(!search) {
        ...
        try{
          const res = await  _userSearchApiAction(search);
          userDispatch({type: 'ADD_DATA', payload: {
              items: res?.data,
              prevURL: res?.links.prev,
              nextURL: res?.links.next,
  ...
  async function getData() {
      try{
          const res = await _userListApiAction();
          userDispatch({type: 'ADD_DATA', payload: {
              items: res?.data,
              prevURL: res?.links.prev,
              nextURL: res?.links.next,
