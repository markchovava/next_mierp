...
import { _expenseBySubsidiaryApiAction, _expenseDeleteApiAction, _expenseListApiAction, _expensePaginationApiAction, _expenseSearchApiAction, _expenseSearchBySubsidiaryApiAction } from '@/actions/ExpenseActions';
...
export default function ExpenseList({ dbData }) {
    ...
    async function getSearchData() {
        if(!search) {
            ...
            try{
            const res = await _expenseSearchBySubsidiaryApiAction(search);
            expenseDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
                prevURL: res?.links.prev,
                nextURL: res?.links.next,
    ...
    async function getData() {
        try{
            const res = await _expenseBySubsidiaryApiAction();
            expenseDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
                prevURL: res?.links.prev,
                nextURL: res?.links.next,
  ...
  return (
    ...
    <section className="w-[100%] lg:overflow-hidden overflow-scroll">
        ...
        <div className='mx-auto w-[96%] text-lg py-2 flex items-center justify-start font-bold bg-slate-200 '>
            <div className='w-[35%] border-r border-white px-3 py-2'>DETAIL</div>
        ...
        {expenseState?.items?.length > 0 ? 
            ...
            <div key={key} className='mx-auto w-[96%] bg-white py-2 flex items-center justify-start border border-slate-300 '>
                <div className='w-[35%] border-r border-slate-300 px-3 py-1'>{i?.detail ?? 'Not Added.'}</div>
