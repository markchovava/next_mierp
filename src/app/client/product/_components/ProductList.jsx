...
import { _productBySubsidiaryApiAction, _productDeleteApiAction, _productListApiAction, 
  _productPaginationApiAction, _productSearchApiAction, 
  _productSearchBySubsidiaryApiAction} from '@/actions/ProductActions';




export default function ProductList({ dbData }) {
    const {productState, productDispatch } = AdminContextState()
    ...
    async function paginateHandler(url) {
        try{
            const res = await _productPaginationApiAction(url)
            productDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
    ...
    async function getSearchData() {
        if(!search) {
          ...
          try{
            const res = await  _productSearchBySubsidiaryApiAction(search);
            productDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
                prevURL: res?.links.prev,
                nextURL: res?.links.next,
    ...
    async function getData() {
        try{
            const res = await _productBySubsidiaryApiAction();
            productDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
                prevURL: res?.links.prev,
                nextURL: res?.links.next,
    ...
    async function deleteData(id) {
        try{
            const res = await _productDeleteApiAction(id);
            if(res.status == 1) {
                await getData();
