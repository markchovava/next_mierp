...
import { _roleDeleteApiAction, _roleListApiAction, _rolePaginationApiAction, _roleSearchApiAction } from '@/actions/RoleActions';
...
export default function RoleList({ dbData }) {
    ...
    async function paginateHandler(url) {
        try{
            const res = await _rolePaginationApiAction(url)
            roleDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
    ...
    async function getSearchData() {
        if(!search) {
          ...
          try{
            const res = await  _roleSearchApiAction(search);
            roleDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
                prevURL: res?.links.prev,
                nextURL: res?.links.next,
    ...
    async function getData() {
        try{
            const res = await _roleListApiAction();
            roleDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
                prevURL: res?.links.prev,
                nextURL: res?.links.next,
    ...
    async function deleteData(id) {
        try{
            const res = await _roleDeleteApiAction(id);
            if(res.status == 1) {
                await getData();
        ...
        {roleState?.items?.length > 0 ? 
            roleState?.items?.map((i, key) => (
                ...
                <div key={key} className='mx-auto w-[96%] bg-white py-2 flex items-center justify-start border border-slate-300 '>
                    <div className='w-[40%] border-r border-slate-300 px-3 py-1'>{i?.name ?? 'Not Added.'}</div>
                    <div className='w-[20%] border-r border-slate-300 px-3 py-1'>{i?.level ?? 'Not Added.'}</div>
