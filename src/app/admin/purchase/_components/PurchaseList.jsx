...
import { _purchaseDeleteApiAction, _purchaseListApiAction, _purchasePaginationApiAction, _purchaseSearchApiAction } from '@/actions/PurchaseActions';
  ...
  const {purchaseState, purchaseDispatch } = AdminContextState()
      ...
      async function getSearchData() {
          if(!search) {
            ...
            try{
              const res = await  _purchaseSearchApiAction(search);
              purchaseDispatch({type: 'ADD_DATA', payload: {
                  items: res?.data,
                  prevURL: res?.links?.prev,
                  nextURL: res?.links?.next,
      ...
      async function getData() {
          try{
              const res = await _purchaseListApiAction();
              purchaseDispatch({type: 'ADD_DATA', payload: {
                  items: res?.data,
                  prevURL: res?.links?.prev,
                  nextURL: res?.links?.next,
          ...
          {purchaseState?.items?.length > 0 ? 
            ...
            <div key={key} className='mx-auto w-[96%] bg-white py-2 flex items-center justify-start border border-slate-300 '>
              ...
              <div className='w-[10%] px-3 py-1 text-xl flex items-center justify-end gap-2'>
                <Link title='View' href={`/admin/purchase/${i?.id}`}> 
