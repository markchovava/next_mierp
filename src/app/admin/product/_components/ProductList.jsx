...
import { _productDeleteApiAction, _productListApiAction, 
  _productPaginationApiAction, _productSearchApiAction } from '@/actions/ProductActions';
...
export default function ProductList({ dbData }) {
    ...
    async function getSearchData() {
        if(!search) {
          ...
          try{
            const res = await  _productSearchApiAction(search);
            productDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
                prevURL: res?.links.prev,
                nextURL: res?.links.next,
    ...
    async function getData() {
        try{
            const res = await _productListApiAction();
            productDispatch({type: 'ADD_DATA', payload: {
                items: res?.data,
                prevURL: res?.links.prev,
                nextURL: res?.links.next,
          ...
          {productState?.items?.length > 0 ? 
            ...
            <div key={key} className='mx-auto w-[96%] bg-white py-2 flex items-center justify-start border border-slate-300 '>
              ...
              <div className='w-[15%] px-3 py-1 text-xl flex items-center justify-end gap-2'>
                ...
                <Link title='Edit' href={`/admin/product/edit/${i?.id}`}> 
