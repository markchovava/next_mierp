...
import { _salesStoreApiAction } from '@/actions/SalesActions';
...
export default function ModalAdd({isModal, setIsModal, getData, products}) {
    ...
    async function postData(){
        ...
        setIsSubmit(false)
         try{
            const res = await _salesStoreApiAction(formData);
            console.log('res', res)
            if(res.status == 1) {
                await getData();
                setData({});
  ...
  return (
    ...
    <AnimatePresence>
        ...
        <section className='mx-auto w-[70%] h-auto bg-white text-black px-6 pt-6 pb-[2rem] rounded-2xl'>
            ...
            <h3 className='text-[2.2rem] font-light text-center mb-3'>Add Sales</h3>
                    ...
                    <div className='col-span-5 group flex items-center justify-start'>
                        ...
                        products.map((i, key) => (
                            <option key={key} value={
                                /* Destructuring an object, while converting it into a JSON */
