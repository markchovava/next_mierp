...
import { _purchaseBySubsidiaryApiAction, _purchaseDeleteApiAction, _purchaseListApiAction, _purchasePaginationApiAction, _purchaseSearchApiAction, _purchaseSearchBySubsidiaryApiAction } from '@/actions/PurchaseActions';
import { formatDate } from '@/utils/formatDate';



export default function PurchaseList({dbData, productsData}) {
  const {purchaseState, purchaseDispatch } = AdminContextState()
      ...
      useEffect(() => {
          purchaseDispatch({type: 'ADD_DATA', payload: {
              items: dbData?.data,
      ...
      async function paginateHandler(url) {
          try{
              const res = await _purchasePaginationApiAction(url)
              purchaseDispatch({type: 'ADD_DATA', payload: {
                  items: res?.data,
      ...
      async function getSearchData() {
          if(!search) {
            ...
            try{
              const res = await  _purchaseSearchBySubsidiaryApiAction(search);
              purchaseDispatch({type: 'ADD_DATA', payload: {
                  items: res?.data,
                  prevURL: res?.links?.prev,
                  nextURL: res?.links?.next,
      ...
      async function getData() {
          try{
              const res = await _purchaseBySubsidiaryApiAction();
              purchaseDispatch({type: 'ADD_DATA', payload: {
                  items: res?.data,
                  prevURL: res?.links?.prev,
                  nextURL: res?.links?.next,
      ...
      async function deleteData(id) {
          try{
              const res = await _purchaseDeleteApiAction(id);
              if(res.status == 1) {
                  await getData();
  ...
  return (
    <>
      ...
      <section className="w-[100%] lg:overflow-hidden overflow-scroll">
        <section className='lg:w-[100%] w-[60rem]'>
          ...
          <div className='mx-auto w-[96%] text-lg py-2 flex items-center justify-start font-bold font-white bg-slate-200 '>
            <div className='w-[25%] border-r border-white px-3 py-2'>SUPPLIER</div>
            <div className='w-[20%] border-r border-white px-3 py-2'>DATE</div>
            <div className='w-[20%] border-r border-white px-3 py-2'>TOTALS</div>
            <div className='w-[25%] border-r border-white px-3 py-2'>DATE</div>
            <div className='w-[10%] px-3 py-2'>ACTION</div>
          </div>

          {/* DETAILS */}
          {purchaseState?.items?.length > 0 ? 
            purchaseState?.items?.map((i, key) => (
      ...
      <section className='mx-8 pt-8 pb-12 flex items-center justify-end gap-3'>
          {/* PREVIOUS */}
          {purchaseState?.prevURL && 
          <button
              title='PREV'
              onClick={() => paginateHandler(purchaseState?.prevURL)}
