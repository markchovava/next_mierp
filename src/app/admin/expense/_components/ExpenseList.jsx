...
import { _expenseDeleteApiAction, _expenseListApiAction, _expensePaginationApiAction, _expenseSearchApiAction } from '@/actions/ExpenseActions';
...
export default function ExpenseList({ dbData }) {
    console.log(dbData, dbData)
    const {expenseState, expenseDispatch } = AdminContextState()
        const [isModal, setIsModal] = useState(false)
        const [search, setSearch] = useState('');
        const [isSearch, setIsSearch] = useState(false);
    
        useEffect(() => {
            expenseDispatch({type: 'ADD_DATA', payload: {
                items: dbData?.data,
        ...
        async function paginateHandler(url) {
            try{
                const res = await _expensePaginationApiAction(url)
                expenseDispatch({type: 'ADD_DATA', payload: {
                    items: res?.data,
        ...
        async function getSearchData() {
            if(!search) {
              ...
              try{
                const res = await  _expenseSearchApiAction(search);
                expenseDispatch({type: 'ADD_DATA', payload: {
                    items: res?.data,
                    prevURL: res?.links.prev,
                    nextURL: res?.links.next,
        ...
        async function getData() {
            try{
                const res = await _expenseListApiAction();
                expenseDispatch({type: 'ADD_DATA', payload: {
                    items: res?.data,
                    prevURL: res?.links.prev,
                    nextURL: res?.links.next,
        ...
        async function deleteData(id) {
            try{
                const res = await _expenseDeleteApiAction(id);
                if(res.status == 1) {
                    await getData();
    ...
    //if(!expenseState.items) { return <Loader /> }
  ...
  return (
    ...
    <section className="w-[100%] lg:overflow-hidden overflow-scroll">
        ...
        <div className='mx-auto w-[96%] text-lg py-2 flex items-center justify-start font-bold bg-slate-200 '>
            <div className='w-[25%] border-r border-white px-3 py-2'>DETAIL</div>
            ...
            <div className='w-[15%] border-r border-white px-3 py-2'>DATE</div>
            <div className='w-[10%] px-3 py-2'>ACTION</div>
        </div>
            
        {/* DETAILS */}
        {expenseState?.items?.length > 0 ? 
            expenseState?.items?.map((i, key) => (
            <div key={key} className='mx-auto w-[96%] bg-white py-2 flex items-center justify-start border border-slate-300 '>
                <div className='w-[25%] border-r border-slate-300 px-3 py-1'>{i?.detail ?? 'Not Added.'}</div>
                <div className='w-[25%] border-r border-slate-300 px-3 py-1'>{i?.total ? '$' + i?.total : 'Not Added.'}</div>
                <div className='w-[25%] border-r border-slate-300 px-3 py-1'>{i?.subsidiary?.name ?? 'Not Added.'}</div>
                <div className='w-[15%] border-r border-slate-300 px-3 py-1'>
                    { formatDate(i?.created_at) ?? 'Not Added'}
                </div>
                <div className='w-[10%] px-3 py-1 text-xl flex items-center justify-end gap-2'>
                    <Link title='View' href={`/admin/expense/${i?.id}`}> 
                    <FaEye className='hover:text-blue-500 duration-150 hover:scale-110 transition-all ease-in'/> 
                    </Link>
                    {/* <Link title='Edit' href={`/admin/expense/edit/${i?.id}`}> 
    ...
    <section className='mx-8 pt-8 pb-12 flex items-center justify-end gap-3'>
        {/* PREVIOUS */}
        {expenseState?.prevURL && 
        <button
            title='PREV'
            onClick={() => paginateHandler(expenseState?.prevURL)}
